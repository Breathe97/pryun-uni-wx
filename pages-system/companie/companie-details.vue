<template>
	<view class="pr-page">
		<pr-navbar title="详情"></pr-navbar>
		<view class="pr-body">
			<pr-card :bg="details.cover">
				<pr-row-cell icon="" title="logo" value="" :arrow="false" border="none">
					<template v-slot:value>
						<image class="user-info-avatar" :src="details.logo" mode=""></image>
					</template>
				</pr-row-cell>
				<pr-row-cell icon="" title="封面背景" value="" :arrow="false">
					<template v-slot:value>
						<image class="user-info-cover" :src="details.cover" mode=""></image>
					</template>
				</pr-row-cell>
				<pr-row-cell icon="" title="名称" :value="details.name" :arrow="true" :touch-copy="true" @click="toSet('name')"></pr-row-cell>
				<pr-row-cell icon="" title="备注" :value="details.comment || ''" :arrow="true" :touch-copy="true" @click="toSet('comment')"></pr-row-cell>
				<pr-row-cell icon="" title="创建者" :value="details.created_user?.nickname" :arrow="true" :touch-copy="false"
					@click="toPage(`/pages-system/user/user-details?id=${details.created_user._id}`)">
				</pr-row-cell>
			</pr-card>
			<pr-card>
				<pr-row-cell icon="" title="邀请码" :value="details.invite_code" :arrow="true" border="none" :touch-copy="true" @click="toSet('invite_code')"></pr-row-cell>
				<pr-row-cell icon="" title="项目数量" :value="details.project_num" :arrow="true" :touch-copy="true" @click="toSet('')"></pr-row-cell>
				<pr-row-cell icon="" title="员工数量" :value="details.user_num" :arrow="true" :touch-copy="true" @click="toSet('')"></pr-row-cell>
			</pr-card>
			<pr-card>
				<pr-row-cell icon="" title="全国统一信用编码" :value="details.unified_credit_code" :arrow="false" border="none" :touch-copy="true"></pr-row-cell>
			</pr-card>
			<pr-card title="其他信息">
			...
			<pr-card tip="确保你的意图正确,否则将会发生意想不到的错误。2">
...
<script setup lang="ts">
	...
	const get_details = async () => {
		try {
			...
			let _res = {}
			_res = await api.companiesGetInfo({ data: _inf })
	...
	// 修改数据
	const companiesPut = async (_inf: any, updateKey: string = '') => {
		try {
			let _res = {}
			_res = await api.companiesPut({ data: _inf }) // 更新数据
			let { code = 0, data, msg = '异常错误' }: any = _res
			if (code !== 200) return uni.showToast({ title: msg, icon: 'none' })
			uni.navigateBack()
	...
	const del = async (delay = 0, index) => {
		...
		try {
			...
			_res = await api.companiesDelete({ data: _inf }) // 更新数据
			let { code = 0, data, msg = '异常错误' }: any = _res
			if (code !== 200) return uni.showToast({ title: msg, icon: 'none' })
		...
		switch (mode) {
			...
			case 'invite_code':
				...
				set_options = [{
					type: 'input',
					list: [
						{ title: details.value.invite_code, value: '', placeholder: '输入内容', type: 'text' },
					]
				}]
				set_end_function = (e) => {
					// console.log(`------->日志输出e:`, e);
					let invite_code = e[0].list[0].title
					let inf = { _id: details.value._id, invite_code }
					companiesPut(inf)
				}
				break;
		}
