<template>
	<view class="pr-page">
		<pr-navbar title="公司" :fixedPerch="true"></pr-navbar>
		<view class="pr-body">
			<pr-card>
				<pr-row-cell icon="" title="" value="" :arrow="true" border="none" :touch-copy="true" @click="toPage('/pages-system/companie/companie-add')">
			...
			<pr-card :title="`共计（${res.total}），上次加载耗时${res.elapsed||'??'}`" :tip="Tip_onReachBottom(loading,res.total,list.length,res.elapsed)">
				<template v-for="(item,index) in list" :key="index">
					<pr-row-cell icon="" :title="item.name" value="" :arrow="true" :border="index===0?'none':'top'" @click="toPage(`/pages-system/companie/companie-details?id=${item._id}`)">
						<template v-slot:icon>
							<pr-icon :name="item.logo" radius="6px"></pr-icon>
						</template>
					</pr-row-cell>
				</template>
			</pr-card>
...
<script setup lang="ts">
	...
	const get_list = async (delay = 0, end_id = '') => {
		...
		try {
			...
			_res = await api.companiesGet({ data: _inf }) // 请求数据
