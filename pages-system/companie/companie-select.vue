		...
		<view class="pr-body">
			...
			<pr-card :title="`共计（${res.total}），上次加载耗时${res.elapsed||'??'}`" :tip="Tip_onReachBottom(loading,res.total,res.list.length,res.elapsed)">
				<template v-for="(item,index) in res.list" :key="index">
					<pr-row-cell icon="" :title="item.name" value="" :arrow="false" :border="index===0?'none':'top'" @click="select(item._id)">
						<template v-slot:value>
							<pr-icon name="hook" :color="Is_selected(select_list,item._id)?'#0097ff':'rgba(0,0,0,0)'"></pr-icon>
						</template>
					</pr-row-cell>
				</template>
			</pr-card>
...
<script setup lang="ts">
	...
	// 获取列表
	const inf: Ref = ref({ page: 1, size: 50, key: '' })
