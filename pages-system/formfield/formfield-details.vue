...
<script setup lang="ts">
	...
	const get_details = async () => {
		...
		let _res = {}
		try {
			_res = await api.formfieldsGetInfo({ data: _inf })
	...
	// 修改数据
	const formfieldsPut = async (_inf, updateKey = '') => {
		try {
			let _res = {}
			_res = await api.formfieldsPut({ data: _inf }) // 更新数据
			let { code = 0, data, msg = '异常错误' }: any = _res
			if (code !== 200) return uni.showToast({ title: msg, icon: 'none' })
			uni.navigateBack()
	...
	const del = async (delay = 0, index) => {
		...
		try {
			...
			_res = await api.formfieldsDelete({ data: _inf }) // 更新数据
			let { code = 0, data, msg = '异常错误' }: any = _res
			if (code !== 200) return uni.showToast({ title: msg, icon: 'none' })
