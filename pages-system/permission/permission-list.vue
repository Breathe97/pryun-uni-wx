<template>
	<view class="pr-page">
		<pr-navbar title="权限" :fixedPerch="true"></pr-navbar>
		<view class="pr-body">
			<pr-card>
				<pr-row-cell icon="" title="" value="" :arrow="true" border="none" @click="toPage('/pages-system/permission/permission-add')">
					<template v-slot:title>
						<view style="color: #0097ff;">添加</view>
					</template>
				</pr-row-cell>
			</pr-card>
			<pr-card tip="由系统自动生成最新的权限列表，冲突的权限名称会保留。">
				<pr-row-cell icon="" title="" value="" :arrow="false" border="none" @click="permissionsGetGenerated(500)">
					<template v-slot:title>
						<view style="color: #0097ff;">重置系统权限</view>
					</template>
				</pr-row-cell>
			</pr-card>
			<pr-card :title="`共计（${res.total}），上次加载耗时${res.elapsed||'??'}`" :tip="Tip_onReachBottom(loading,res.total,list.length,res.elapsed)">
				<template v-for="(item,index) in list" :key="index">
					<pr-row-cell icon="" :title="item.name" value="" :arrow="true" :border="index===0?'none':'top'"
						@click="toPage(`/pages-system/permission/permission-details?id=${item._id}`)">
					</pr-row-cell>
				</template>
			</pr-card>
		</view>
...
<script setup lang="ts">
	...
	const res: Ref = ref({ total: 0, list: [] })
	const get_list = async (delay = 0, end_id = '') => {
		...
		try {
			...
			_res = await api.permissionsGet({ data: _inf }) // 请求数据
	...
	// 自动生成权限
	const permissionsGetGenerated = async (delay = 0) => {
		let _res = {}
		loadingShow.value = true
		await new Promise(a => setTimeout(() => a(true), delay))
		try {
			_res = await api.permissionsGetGenerated() // 请求数据
			// console.log(`------->日志输出_res:`, _res);
		} catch (e) {
			console.log(`------->日志输出e:`, e);
			uni.showToast({ title: '异常错误', icon: 'none' })
		...
		let { code = 0, data, msg = '异常错误' }: any = _res
		if (code !== 200) return uni.showToast({ title: msg, icon: 'none' })
		uni.showToast({ title: '刷新已完成', icon: 'none' })
		get_list()
	}
</script>
