	...
	<view class="pr-page">
		...
		<view class="pr-body">
			<pr-card>
				...
				<pr-row-cell icon="" title="创建者" :value="details.created_user?.nickname || '未知'" :arrow="true" :touch-copy="true"
					@click="toPage(`/pages-system/user/user-details?id=${details.created_user._id}`)"></pr-row-cell>
			</pr-card>
			<pr-card>
				<pr-row-cell icon="" title="权限设置" value="" :arrow="true" border="none" @click="toSet('permissions')">
					<template v-slot:value>
						<view>{{ details.permissions ? details.permissions.length :'' }}</view>
					</template>
				</pr-row-cell>
			</pr-card>
			<pr-card>
			...
			<pr-card title="其他信息">
				<pr-row-cell icon="" title="创建时间" :value="timeFormat(details.created_at,'yyyy-mm-dd hh:MM:ss')" border="none" :arrow="false" :touch-copy="true"></pr-row-cell>
...
<script setup lang="ts">
	...
	const get_details = async () => {
		...
		let _res = {}
		try {
			_res = await api.rolesGetInfo({ data: _inf })
	...
	// 修改数据
	const rolesPut = async (_inf, updateKey = '') => {
		try {
			let _res = {}
			_res = await api.rolesPut({ data: _inf }) // 更新数据
			let { code = 0, data, msg = '异常错误' }: any = _res
			if (code !== 200) return uni.showToast({ title: msg, icon: 'none' })
			uni.navigateBack()
	...
	const del = async (delay = 0, index) => {
		...
		try {
			...
			_res = await api.rolesDelete({ data: _inf }) // 更新数据
			let { code = 0, data, msg = '异常错误' }: any = _res
			if (code !== 200) return uni.showToast({ title: msg, icon: 'none' })
	...
	const toSet = (mode = '') => {
		...
		switch (mode) {
			...
			case 'permissions':
				...
				set_options = [{
					type: 'select-list',
					path: '/pages-system/permission/permission-select',
					list: Array.from(details.value.permissions, (item: any) => item.key),
				}]
				set_end_function = (e) => {
					// console.log(`------->日志输出e:`, e);
					let permissions = e
					let inf = { _id: details.value._id, permissions }
					rolesPut(inf)
				}
				break;
		}
