<template>
	<view class="pr-page">
		...
		<view class="pr-body">
			<pr-card>
				<pr-row-cell icon="" title="" value="" :arrow="true" border="none" :touch-copy="true" @click="toPage('/pages-system/role/role-add')">
			...
			<pr-card :title="`共计（${res.total}），上次加载耗时${res.elapsed||'??'}`" :tip="Tip_onReachBottom(loading,res.total,list.length,res.elapsed)">
				<template v-for="(item,index) in list" :key="index">
					<pr-row-cell icon="" :title="item.name" value="" :arrow="true" :border="index===0?'none':'top'" @click="toPage(`/pages-system/role/role-details?id=${item._id}`)">
					</pr-row-cell>
				</template>
			</pr-card>
		</view>
