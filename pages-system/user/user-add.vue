<template>
	<view class="pr-page">
		<pr-navbar title="新增用户" :fixedPerch="true"></pr-navbar>
		<view class="pr-body">
			<pr-card title="用户名">
				<pr-row-cell icon="" title="" value="" :arrow="false" border="none" :hover="false">
					<view style="width: 100%;">
						<pr-input v-model="inf.username" type="text" :hold-keyboard="true">
						</pr-input>
					</view>
				</pr-row-cell>
			</pr-card>
			<pr-card title="密码">
				<pr-row-cell icon="" title="" value="" :arrow="false" border="none" :hover="false">
					<view style="width: 100%;">
						<pr-input v-model="inf.password" type="text" :hold-keyboard="true">
						</pr-input>
					</view>
				</pr-row-cell>
			</pr-card>
			<pr-card title="确认密码">
				<pr-row-cell icon="" title="" value="" :arrow="false" border="none" :hover="false">
					<view style="width: 100%;">
						<pr-input v-model="inf.passwordAgain" type="text" :hold-keyboard="true">
						</pr-input>
					</view>
				</pr-row-cell>
			</pr-card>
			<pr-card title="昵称">
				<pr-row-cell icon="" title="" value="" :arrow="false" border="none" :hover="false">
					<view style="width: 100%;">
						<pr-input v-model="inf.nickname" type="text" :hold-keyboard="true">
						</pr-input>
					</view>
				</pr-row-cell>
			</pr-card>
...
<script setup lang="ts">
	...
	const ok = async (delay = 0) => {
		...
		_inf['passwordAgain'] = md5(_inf['passwordAgain'])
		let _res = {}
		loading.value = true
		await new Promise(a => setTimeout(() => a(true), delay))
		try {
			_res = await api.usersPost({ data: _inf }) // 请求数据
